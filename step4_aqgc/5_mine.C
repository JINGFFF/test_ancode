...
int main(int argc, char** argv){
   ...
   for(int i = 0; i<14; i++){
      ...
      //muon
      system("text2workspace.py -m 126 ./"+datacard_dir+"/all_" + year + "_" + which_channel+ ".txt -o " + outdir + "/os_" + year + "_" + op[i] + ".root -P HiggsAnalysis.CombinedLimit.OneParameterPhysicsModel:my_1d_model --PO range_param=[" + lower[i] + "," + upper[i] + "] --PO scaling_filename="+outdir+"/" + year + "_" + op[i] + "_scaling_refined.root");
      system("combine " + outdir + "/os_" + year + "_" + op[i] + ".root -M MultiDimFit -P param --floatOtherPOIs=0 --algo=grid --points=5000 --cminDefaultMinimizerStrategy=2");
