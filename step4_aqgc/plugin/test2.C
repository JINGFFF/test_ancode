...
void test::Loop(TDirectory * dir, TTree * tree)
{
   ...
   while (fReader.Next()) {
      ...
      muon_cut = fill_HLT_muon==1 && abs(fill_lep)==13 && fill_ngoodmus==1 && fill_ngoodeles==0 &&(fill_nloosemus+fill_nlooseeles)==1
      ...
      electron_cut = fill_HLT_electron==1 && abs(fill_lep)==11 && fill_ngoodmus==0 && fill_ngoodeles==1 &&(fill_nloosemus+fill_nlooseeles)==1
      ...
      //muon_cut = 1;
      //electron_cut = fabs(fill_Mla - 91.2) > 10;
...
//cout<<"pass"<<endl;
      ...
      init_sf();
      // cross section SF
      if(m_sample == "WGJJaqgc")      { cross_section_SF=1000.*1.864    *fabs(*theWeight)/(*theWeight); }
      ...
      // fill histogram
      if(m_type == "data") weight = 1, weight_up = 1, weight_down = 1;
         ...
         weight_up = cross_section_SF * pu_weight_SF * photon_ID_up_SF * electron_ID_up_SF * electron_Reco_up_SF * electron_HLT_up_SF *muon_ID_up_SF * muon_iso_up_SF * muon_HLT_up_SF * btag_jet1_up_SF * btag_jet2_up_SF;
         weight_down = cross_section_SF * pu_weight_SF * photon_ID_low_SF * electron_ID_low_SF * electron_Reco_low_SF * electron_HLT_low_SF *muon_ID_low_SF * muon_iso_low_SF * muon_HLT_low_SF*btag_jet1_low_SF * btag_jet2_low_SF;

      }

      //cout<<"barrel pass "<<*electron_ID_SF<<" "<<*electron_Reco_SF<<" "<<*electron_HLT_SF<<endl;
      bool ipass;
      //if (!(m_type == "mc")) ipass = (fill_ptlep1>30 && fill_photonet>100 && fill_Mjj>1500 && fill_deltaeta>2.5);
      // else   ipass =                 (fill_ptlep1>30 && fill_photonet>100 && fill_Mjj>1500 && fill_deltaeta>2.5);
      //if(ipass>0) {
         PTA1->Fill(fill_Mva, weight);
         PTA1_up->Fill(fill_Mva, weight_up);
         PTA1_down->Fill(fill_Mva, weight_down);
      //}   
